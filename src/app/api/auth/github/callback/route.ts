...
import { verifyOAuthState, exchangeGitHubCode, findOrCreateSocialUser, linkSocialAccount, createMergeIntent } from "@/lib/oauth";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (payload.mode === "link" && payload.linkUserId) {
      const result = await linkSocialAccount(info, "github", payload.linkUserId);
