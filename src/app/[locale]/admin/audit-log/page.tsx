...
export default function AuditLogPage() {
  ...
  const locale = (isLocale(rawLocale) ? rawLocale : "fa") as Locale;
  const dict = getDictionary(locale);
  const d = dict.admin.auditLog;
  ...
  const fmtDate = (iso: string) => {
    return new Date(iso).toLocaleString(locale === "en" ? "en-US" : "fa-IR", {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
      hour: "2-digit",
    ...
    <div>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        >
          ...
          {ENTITIES.map((e) => (
            <option key={e} value={e}>{d.entities[e as keyof typeof d.entities] ?? e}</option>
          ))}
        </select>
        <select
          value={filterAction}
        ...
        >
          ...
          {ACTIONS.map((a) => (
            <option key={a} value={a}>{d.actions[a as keyof typeof d.actions] ?? a}</option>
          ))}
        </select>
      </div>
                  ...
                  <tr key={log.id} className="font-[850] text-[var(--text-3)]">
                    ...
                    <td className="px-4 py-3">
                      <span className="bg-[var(--soft)] text-[var(--primary)] rounded-full px-2.5 py-1 text-[11px] font-[950]">
                        {d.actions[log.action as keyof typeof d.actions] ?? log.action}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="bg-[var(--soft)] text-[var(--primary)] rounded-full px-2.5 py-1 text-[11px] font-[950]">
                        {d.entities[log.entity as keyof typeof d.entities] ?? log.entity}
                      </span>
                    </td>
